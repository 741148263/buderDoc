---
title: PicGo图床安装
tags:
  - test1
  - test2
categories:
  - test3
  - test4
toc: true
recommend: 1
keywords: categories-java
uniqueId: 2026-04-27 15:56:01/pic_go图床.html
mathJax: false
date: 2026-04-27 23:56:01
thumbnail:
---
> 摘要
记录PicGO图床安装和配置的过程，方便后续系统重装后环境恢复使用
<!-- more -->
# PicGo

## PicGo图床安装向导

1. [PicGo Github](https://github.com/Molunerfinn/PicGo/releases)
2. [PigGo 官网](https://docs.picgo.app/zh/gui/guide/getting-started)


## PicGo配置

1. 修改语言显示
  ![修改语言配置](https://raw.githubusercontent.com/741148263/MyPicBed/main/typora/20260428000933054.png)

2. github图床配置
  ![alt text](image.png)